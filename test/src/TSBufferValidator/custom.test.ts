...
describe('CustomType Validate', function () {
    it('Custom', function () {
        ...
        assert.deepStrictEqual(validator.validate(12345, 'a/b').errMsg, ValidateResultUtil.error(ErrorType.TypeError, 'string', 'number').errMsg);
        assert.deepStrictEqual(validator.validate('666666', 'a/b').errMsg, 'This is not a valid email');
    })
})